...
// --- TDS calibration ---
float TDS_FACTOR = 0.5f; // 0.5 (air minum). Coba 0.65 kalau mau mendekati
                         // handheld meter komersial
...
void loop() {
  ...
  if (now - lastSendTime >= SEND_INTERVAL) {
    ...
    Serial.printf(
        "TDS: %.1f ppm | EC: %.1f uS/cm | V=%.3f (comp=%.3f) | T=%.1f°C\n",
        tdsValue, ecValue, averageVoltage, compensationVoltage,
        temperature_bawah);
