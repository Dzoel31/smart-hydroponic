...
try:
    from ..utils.models import SensorData
except ImportError:
    from utils.models import SensorData

CONFIG = {
    "coap_url": "coap://localhost:5683/coap/hydroponics/plant",
    "websocket_url": "ws://127.0.0.1:8000/smart-hydroponic/api/v2/hydroponics/ws/plant",
...
def create_progress_bar(duration_seconds, enabled):
    ...
    if tqdm is None:
        ...
        return None
    return tqdm(total=duration_seconds, unit="s", desc="esp32_plant", leave=False)


def generate_dummy_data(message_id: int):
    return SensorData(
        message_id=message_id,
        moisture1=random.randint(30, 90),
        moisture2=random.randint(30, 90),
        moisture3=random.randint(30, 90),
        moisture4=random.randint(30, 90),
        moisture5=random.randint(30, 90),
        moisture6=random.randint(30, 90),
        flowrate=round(random.uniform(0.0, 4.0), 2),
        total_litres=round(random.uniform(0.0, 150.0), 2),
        distance_cm=round(random.uniform(2.0, 40.0), 2),
    ).model_dump()
...
def create_log_file(protocol, node, interval_text, duration_text):
    ...
    file_name = (
        f"plant_{protocol}_{node}_{interval_text}_{duration_text}_{int(time.time())}.csv"
    )
...
async def run_simulation(
    ...
    run_id = f"plant-{int(time.time() * 1000)}"
...
def run_interactive():
    while True:
        ...
        node = "plant"
        if protocol == "websocket":
            node = input("Enter WebSocket node name (default: plant): ").strip() or "plant"
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="ESP32 plant simulator")
    parser.add_argument("--protocol", choices=["coap", "websocket"], help="Communication protocol")
    parser.add_argument("--node", default="plant", help="Node suffix for websocket path")
