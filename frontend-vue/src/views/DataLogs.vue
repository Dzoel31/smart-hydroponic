      ...
      <section class="table-card">
            ...
            <tbody>
              <tr v-for="(item, index) in filteredData" :key="index">
                ...
                <template v-if="showCol('actuator')">
                  ...
                  <td>
                    <span
                      :class="['badge', item.automation_status ? 'on' : 'off']"
                    >
              ...
              <tr v-if="isLoading">
                <td colspan="25" style="text-align: center; padding: 40px">
                  Memuat data...
                </td>
              </tr>
        ...
        <div class="pagination-outer-container">
          ...
          <div class="pagination-status">
            Showing
            <strong>{{ dataRange.start }}-{{ dataRange.end }}</strong> of
...
const mapHydroponicDataToTableItem = (
  item: HydroponicOut,
): HydroponicTableItem => {
  return {
    id: item.dataid ?? "-",
...
const exportData = () => {
  ...
  const dataToExport = filteredData.value.map(
    (item): Record<string, unknown> => {
      ...
      if (showCol("environment")) {
        ...
        exportItem["TDS (ppm)"] =
          item.tds !== null ? Number(item.tds.toFixed(2)) : null;
