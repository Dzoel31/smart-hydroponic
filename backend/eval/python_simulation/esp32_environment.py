...
try:
    from ..utils.models import EnvironmentData
except ImportError:
    from utils.models import EnvironmentData

CONFIG = {
    "coap_url": "coap://localhost:5683/coap/hydroponics/environment",
    "websocket_url": "ws://127.0.0.1:8000/smart-hydroponic/api/v2/hydroponics/ws/environment",
...
def create_progress_bar(duration_seconds, enabled):
    ...
    if tqdm is None:
        ...
        return None
    return tqdm(total=duration_seconds, unit="s", desc="esp32_environment", leave=False)


def generate_dummy_data(message_id: int):
    return EnvironmentData(
        message_id=message_id,
        ph=round(random.uniform(5.5, 7.2), 2),
        tds=round(random.uniform(300.0, 1200.0), 2),
        temperature_atas=round(random.uniform(24.0, 35.0), 2),
        temperature_bawah=round(random.uniform(23.0, 34.0), 2),
        humidity_atas=round(random.uniform(45.0, 90.0), 2),
        humidity_bawah=round(random.uniform(45.0, 90.0), 2),
    ).model_dump()


def create_log_file(protocol, node, interval_text, duration_text):
    logs_dir = PROJECT_ROOT / "data" / "simulation_logs"
    logs_dir.mkdir(parents=True, exist_ok=True)
    file_name = (
        f"environment_{protocol}_{node}_{interval_text}_{duration_text}_{int(time.time())}.csv"
    )
...
async def send_coap_request(payload, verbose):
    started_at = time.perf_counter()
    context = await Context.create_client_context()
    request = Message(code=PUT, payload=json.dumps(payload).encode("utf-8"), uri=CONFIG["coap_url"])
...
async def run_simulation(
    ...
    run_id = f"environment-{int(time.time() * 1000)}"
    log_file, log_writer, log_path = create_log_file(protocol, node, interval_text, duration_text)
    ...
    try:
        ...
        while (time.monotonic() - start_time) < duration_seconds:
            ...
            payload = generate_dummy_data(message_id=msg_id)

            if protocol == "coap":
                telemetry = await send_coap_request(payload, verbose)
            else:
...
def run_once(protocol, node, interval_text, duration_text, verbose, show_progress):
...
def run_interactive():
    while True:
        ...
        node = "environment"
        if protocol == "websocket":
            node = input("Enter WebSocket node name (default: environment): ").strip() or "environment"

        print(
            f"\nRunning {protocol} simulation for {duration_text} with {interval_text} interval..."
        )
        try:
            run_once(protocol, node, interval_text, duration_text, verbose=False, show_progress=False)
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="ESP32 environment simulator")
    parser.add_argument("--protocol", choices=["coap", "websocket"], help="Communication protocol")
    parser.add_argument("--node", default="environment", help="Node suffix for websocket path")
