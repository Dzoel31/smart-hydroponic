...
const char *WEBSOCKET_URL = "ws://192.168.1.10:5000/ws/actuator"; // contoh: "ws://
...
void setup()
{
    ...
    lastMoistureAvg = 65.0f;
    lastTemperatureAvg = 28.0f;
...
void generateDummyInputs()
{
    lastMoistureAvg += ((float)random(-20, 21)) * 0.25f;
    lastTemperatureAvg += ((float)random(-12, 13)) * 0.12f;

    if (lastMoistureAvg < 35.0f)
        lastMoistureAvg = 35.0f;
    if (lastMoistureAvg > 90.0f)
        lastMoistureAvg = 90.0f;

    if (lastTemperatureAvg < 20.0f)
        lastTemperatureAvg = 20.0f;
    if (lastTemperatureAvg > 40.0f)
        lastTemperatureAvg = 40.0f;

    if (automationStatus == 1)
    {
        pumpStatus = (lastMoistureAvg < MOISTURE_THRESHOLD) ? 1 : 0;
        lightStatus = (lastTemperatureAvg < TEMPERATURE_THRESHOLD) ? 1 : 0;
        updateOutputs();
    }
}
...
void onMessageCallback(WebsocketsMessage message)
{
    ...
    {
        lastMoistureAvg = data["moisture_avg"].as<float>();
