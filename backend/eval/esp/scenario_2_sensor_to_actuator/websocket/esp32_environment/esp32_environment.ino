...
const char *server_url = "ws://172.25.21.234:8000/smart-hydroponic/api/v2/hydroponics/ws/environment-data"; // contoh: "ws://
...
void onWebsocketMessage(WebsocketsMessage message)
{
    ...
    {
        Serial.println("[WS] Non-JSON message received");
        return;
    }
    ...
    if (doc.containsKey("status") && String(doc["status"].as<const char *>()) == "ack")
    {
        ...
        Serial.printf("[S1_METRIC] Seq: %d | Latency: %lu ms\n", ackSeq, latency);
        return;
    }
...
void loop()
{
    ...
    if (now - lastSendTime >= SEND_INTERVAL)
    {
        ...
        if (isnan(temperature_bawah))
            temperature_bawah = 0; // default netral utk kompensasi
        ...
        float ecValue = (133.42f * powf(compensationVoltage, 3) - 255.86f * powf(compensationVoltage, 2) + 857.39f * compensationVoltage);
        if (ecValue < 0)
            ecValue = 0;
